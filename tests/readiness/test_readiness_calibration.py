...
@pytest.mark.parametrize(
    ...
    [
        (
            [
                "# Spec: safe Korean webhook replay rejection",
                "",
                "## Requirements",
                "",
                "- WebhookReceiverService는 raw body 서명과 timestamp를 검증한 뒤 event_id를 저장한다.",
                "- 엔드포인트 URL을 아는 호출자는 신뢰하지 않는다.",
                "- 같은 event_id가 다시 오면 다시 처리하지 않는다.",
                "",
                "## Acceptance Criteria",
                "",
                "- [ ] 유효한 서명과 fresh timestamp는 이벤트를 한 번만 dispatch한다.",
                "- [ ] 서명 누락은 dispatch 전에 401을 반환한다.",
                "- [ ] 중복 event_id는 기존 처리 결과를 반환한다.",
                "",
                "## Error Cases",
                "",
                "- 서명 누락은 401을 반환한다.",
                "- timestamp 허용 범위 초과는 401을 반환한다.",
                "- 중복 event_id는 저장된 결과를 반환한다.",
            ],
            [
                "# Technical Design: safe Korean webhook replay rejection",
                "",
                "## Architecture",
                "",
                "- WebhookReceiverService가 raw body 서명 검증, timestamp 허용 범위, event_id dedupe를 담당한다.",
                "- EventRepository는 event_id, signature_hash, received_at, status, processing_result를 저장한다.",
                "",
                "## Data Flow",
                "",
                "1. Receiver가 raw body와 signature header를 읽는다.",
                "2. Service가 서명과 timestamp를 검증한다.",
                "3. Service가 event_id를 dispatch 전에 예약한다.",
                "4. 중복 event_id는 side effect 없이 저장된 결과를 반환한다.",
                "",
                "## State",
                "",
                "- 이벤트 상태는 received, verified, processed, rejected이다.",
                "- processed와 rejected는 중복 event_id 처리에서 terminal이다.",
                "",
                "## Failure Handling",
                "",
                "- 서명 불일치는 401을 반환한다.",
        ...
        (
            [
                "# Spec: safe Korean payment retry reconciliation",
                "",
                "## Requirements",
                "",
                "- PaymentService는 모든 capture 요청에 merchant_id, order_id, amount, currency, idempotency_key를 요구한다.",
                "- 같은 idempotency_key와 같은 금액은 원래 결제 결과를 반환한다.",
                "- 같은 idempotency_key와 다른 금액은 409 conflict를 반환한다.",
                "- 게이트웨이 타임아웃 재시도는 새 요청으로 처리하지 않는다.",
                "",
                "## Acceptance Criteria",
                "",
                "- [ ] idempotency_key 누락은 게이트웨이 호출 전에 400을 반환한다.",
                "- [ ] 같은 key 재시도는 원래 결제 결과를 반환한다.",
                "- [ ] 다른 금액의 같은 key 재시도는 409를 반환한다.",
                "",
                "## Error Cases",
                "",
                "- idempotency_key 누락은 400을 반환한다.",
                "- idempotency conflict는 409를 반환한다.",
                "- 게이트웨이 timeout은 reconcile_pending을 저장한다.",
            ],
            [
                "# Technical Design: safe Korean payment retry reconciliation",
                "",
                "## Architecture",
                "",
                "- PaymentService가 idempotency 조회, gateway capture, 409 conflict response, timeout reconciliation을 담당한다.",
                "- PaymentRepository는 merchant_id, order_id, idempotency_key, amount, currency, gateway_request_id, state를 저장한다.",
                "",
                "## Data Flow",
                "",
                "1. Service가 merchant_id, order_id, amount, currency, idempotency_key를 검증한다.",
                "2. Service가 gateway 호출 전에 idempotency record를 예약한다.",
                "3. 같은 key 재시도는 저장된 결과를 반환한다.",
                "4. timeout은 reconcile_pending으로 이동하고 provider 상태 확인 전까지 추가 capture를 막는다.",
                "",
                "## State",
                "",
                "- 결제 상태는 requested, capture_pending, captured, failed, reconcile_pending이다.",
                "- captured, failed, reconcile_pending은 즉시 재시도 처리에서 terminal이다.",
                "",
                "## Failure Handling",
                "",
                "- idempotency conflict는 409를 반환한다.",
                "- 게이트웨이 timeout은 202와 reconcile_pending을 반환한다.",
            ],
            "Payment idempotency contract is ambiguous",
        ),
        (
            [
                "# Spec: safe English webhook replay rejection",
                "",
                "## Requirements",
                "",
                "- WebhookReceiverService verifies the raw request body signature before parsing JSON.",
                "- Knowing the endpoint URL is not enough; signature verification is required.",
                "- The same event_id is not processed again.",
                "- Duplicate event_id returns the original processing result.",
                "",
                "## Acceptance Criteria",
                "",
            ...
            [
                "# Technical Design: safe English webhook replay rejection",
                "",
                "## Architecture",
                "",
        ...
        (
            [
                "# Spec: safe English payment retry reconciliation",
                "",
                "## Requirements",
                "",
                "- Payment requests require idempotency_key scoped to merchant_id and order_id.",
                ...
                "- A gateway timeout is not treated as a new request when the caller retries.",
                "- Payment duplicate settlement does not settle duplicates after reconciliation; gateway calls remain blocked.",
                "",
                "## Acceptance Criteria",
                "",
                "- [ ] Missing idempotency_key returns 400 before calling the gateway.",
                "- [ ] Duplicate same-key request returns the original payment result.",
            ...
            [
                "# Technical Design: safe English payment retry reconciliation",
                "",
                "## Architecture",
                "",
                "- PaymentService owns idempotency lookup, gateway capture, 409 conflict response, and timeout reconciliation.",
...
def test_safe_negated_phrasing_does_not_trigger_new_variant_blockers(
    tmp_path: Path,
    spec_lines: list[str],
    design_lines: list[str],
    unexpected_title: str,
) -> None:
    ...
    assert payload["readiness"]["status"] in {"ready", "ready_with_warnings"}
    assert payload["summary"]["critical"] == 0
    assert unexpected_title not in {issue["title"] for issue in payload["issues"]}
