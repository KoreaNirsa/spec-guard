...
- English specs: calibrated against the recorded v0.4.1 99-case gate-only benchmark. The current fixture source contains 100 selected English cases, with 1 new ready/reference result pending the next benchmark refresh.
...
- Korean-only product prose: initial deterministic support for explicit unsafe wording in the recorded v0.4.1 Korean 99-case layer. The current fixture source contains 100 selected Korean cases, with 1 new ready/reference result pending the next benchmark refresh. This is not a full Korean production-support claim.
