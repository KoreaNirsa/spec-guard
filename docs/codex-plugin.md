...
| missing spec package | Resolve the user path, current directory, or non-excluded `**/specs/*/spec.md` candidates before running the CLI. | Report `missing_spec_package`, show supported examples such as `specs/<feature>/spec.md` and `backend/specs/<feature>/spec.md`, and ask the user to create or point to a package without implying SpecGuard reviewed anything. |
| non-package requirement documents found | Read `draft_sources[]` only when discovery found no package, or when the user explicitly provided one of those files. | List the source options and ask whether to create a draft package; require explicit approval and do not import, modify, or review the documents automatically. |
| `spec.md`-only or partial package | Run `specguard run <package> --allow-partial --no-llm --no-follow-up` only after resolving a package with a non-empty `spec.md`. | Read the fresh readiness JSON and report missing package files or required spec sections as Critical blockers; do not expect or expose `implementation-output.md`. |
...
| additional authored Markdown reviewed | Read reviewed paths from `input.artifacts[]`. | Show standard package files separately from additional notes such as `domain-rules.md` and `api-notes.md`; bound long lists and exclude generated reports, handoff, tests, contracts, and cache paths. |
| normalized Next Action summary | Read stable readiness fields and generated-file existence. | Print status, counts, top finding, report path, handoff path, edit target, and rerun command once; use `unavailable` for missing report or handoff files and do not repeat them in secondary guidance. |
| automatic human report language | Determine whether Korean or English clearly dominates the active conversation and pass a process-only language hint only for a clear result. | Prefer the active conversation, otherwise inspect authored spec content, and use English for mixed, empty, inconclusive, or unsupported input; preserve stable JSON keys and enums. |
...
| readiness Markdown summary | Read the leading `## Summary` block for concise display while keeping `readiness-review.json` authoritative. | Show status, counts, top blocker or warning, report context, edit target, conditional handoff guidance, and rerun command; retain links to the detailed finding sections. |
| stale readiness report | Compare current authored Markdown source files to `input.artifacts[]`, then compare source mtimes to `readiness-review.json`. | Report `stale_review`, show previous readiness report paths as historical context only, do not expose `implementation-output.md` as a handoff, and ask the user to rerun SpecGuard. |
...
| mid-pipeline failure or generated-artifact write failure | Read the current `.specguard/run-state.json`; use its failed stage and diagnostics, not an older report or handoff. | Report `pipeline_failed`, keep `relevant_files` limited to the run-state file, preserve authored Markdown, and provide the safe rerun command. |
