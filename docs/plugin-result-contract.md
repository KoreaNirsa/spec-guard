...
When a resolved package has a non-empty `spec.md` but lacks normal package structure, plugin workflows may run `specguard run <package> --allow-partial --no-llm --no-follow-up`. This explicit mode writes `readiness-review.json` and `readiness-review.md` without generating technical design, tests, contracts, or `implementation-output.md`.

The readiness JSON remains the machine-readable source of truth. It sets `review_input.partial_package: true` and reports missing `discovery.md`, `plan.md`, `tasks.md`, `constitution.md`, `checklists/spec-readiness.md`, `technical-design.md`, and required spec sections through `issues[]`. Missing structure is a Critical blocker, so a partial package cannot be treated as implementation-ready. The default `specguard run` command remains strict and continues to use pre-review validation state for invalid generated packages.

## Pre-Review Run State JSON

When spec-basis validation or technical-design validation stops `specguard run <package>` before SpecGuard Review, it writes `<package>/.specguard/run-state.json`. A failure in any later pipeline stage writes the same file with `state: "pipeline_failed"` and the failed stage. Missing CLI, preflight, and external command failures may still stop before review without writing this file. This file is generated operational state, not an authored spec input, and it does not replace or migrate `readiness-review.json`.
...
| `state` | Plugin-facing state. Pre-review validation uses `validation_failed_before_review`; a failure after a pipeline stage begins uses `pipeline_failed`. |
| `failure_category` | Failure category to report without terminal parsing. |
| `failed_stage` | Pipeline stage that stopped the run, such as `validation`, `readiness_review`, `tests`, or `contract_validation`. |
...
| `relevant_files[]` | Current files for this resolved state. For pre-review and mid-pipeline failures this includes `.specguard/run-state.json` only. |
| `next_action` | Concise safe next action. |

For `validation_failed_before_review` and `pipeline_failed`, do not point to `implementation-output.md` as current, relevant, or actionable, even if an older handoff file exists. Generated reports, tests, contracts, handoff files, run-state files, and Grill outputs are replaced atomically; a failed write must not expose a truncated target file.
...
- when the run-state file passes those freshness checks, report its structured `state` (`validation_failed_before_review` or `pipeline_failed`) with its `failure_category`, `failed_stage`, `messages[]`, `next_steps[]`, and `package_path`;
...
3. Current `.specguard/run-state.json` for a pre-review or mid-pipeline failure: use its `state` (`validation_failed_before_review` or `pipeline_failed`).
...
| `pipeline_failed` | A generated-artifact, provider, verification, contract, or handoff stage fails after the run begins. | `.specguard/run-state.json` only. Existing reports and handoff files are diagnostics, never current implementation input. |
